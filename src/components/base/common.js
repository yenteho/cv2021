...
const bounce_height = 'calc(600px - 100vh)'

const translateShadow = keyframes`
  0% {transform: translateY(0) ;box-shadow: 40px 40px  10px #444466}
  100% {transform: translateY(${bounce_height});box-shadow:40px 350px  250px #444466}
`
const smash = keyframes`
  0% {transform: scaleY(.7)}
  15%  {transform: scaleY(1)}`
const rotateImg = keyframes`100% {transform:rotate(360deg)}`
export const TranslateShadow = styled.div`
  animation-name: ${translateShadow};
  animation-duration: ${(props) => props.bounceDuration};
...
export const Light = styled.div`
  ...
  background-image: radial-gradient(
    circle at 20% 20%,
    ${(props) => props.theme.color_B2_OP70},
    ${(props) => props.theme.color_B2_OP50} 10%,
    transparent 30%,
    ${(props) => props.theme.color_B3} 85%
  );
  position: absolute;
  z-index: 1;
`
export const RotateImg = styled.div`
  ...
  animation-duration: calc(${(props) => props.bounceDuration} * 5.3);
  animation-iteration-count: infinite;
  animation-timing-function: linear;
  svg {
    width: ${(props) => props.ballSize};
    height: ${(props) => props.ballSize};
    // color: white;
  }
`
export const Smash = styled.div`
...
`
export const BounceBall = ({ icon, move, bounceDuration, ballSize }) => (
  <Suport move={move}>
    <Smash bounceDuration={bounceDuration} ballSize={ballSize}>
      <TranslateShadow bounceDuration={bounceDuration}>
        <Light />
        <RotateImg bounceDuration={bounceDuration} ballSize={ballSize}>
          {icon}
        </RotateImg>
      </TranslateShadow>
    </Smash>
